...
def drive(base_url, model, trace, request_rate, max_tokens, concurrency):
    """Fire the trace at ~request_rate req/s (Poisson) with a thread pool; collect per-req timings."""
    import numpy as np
...
def _write_outputs(model, configs, meta, quiet=False):
    ...
    rows = [
        ...
        f"{(on['throughput_tok_s'] - off['throughput_tok_s']) / max(off['throughput_tok_s'],1e-9)*100:+.0f}\\% \\\\",
    ]
...
def _plot(model, on, off):
    ...
    axB.bar(["ON", "OFF"], [on["throughput_tok_s"], off["throughput_tok_s"]],
            color=[PALETTE["red"], PALETTE["gray"]], width=0.6)
...
def _mock_config(trace, enable, max_tokens):
    """Dry-run: synthesize plausible timings (caching ON prefills less -> lower TTFT/higher tput)."""
    import numpy as np
...
def main() -> None:
    ...
    ap.add_argument("--pad-tokens", type=int, default=200, help="template length (longer = more prefill saved)")
    ap.add_argument("--request-rate", type=float, default=20.0, help="req/s (Poisson); 0 = as fast as possible")
    ...
    ap.add_argument("--corpus-file", default="", help="use real prompts from a file instead of templates")
    ap.add_argument("--extra-arg", action="append", default=[], help="extra vllm serve arg (repeatable)")
    ap.add_argument("--dry-run", action="store_true", help="no GPU/vLLM; mock timings to test the harness")
    args = ap.parse_args()
    ...
    meta = {"n_prompts": len(trace), "n_templates": args.n_templates, "shift": args.shift,
            "request_rate": args.request_rate, "workload": args.corpus_file or "templates"}
    ...
    for enable in (True, False):
        ...
        if args.dry_run:
            ...
            continue
        proc = start_server(args.model, args.port, enable, args.gpu_memory_utilization, args.extra_arg)
        try:
            ...
            results, wall = drive(base_url, args.model, trace, args.request_rate,
            ...
            configs[key] = agg
            print(f"  caching={enable}: {agg['n_ok']} ok, ttft_p50={agg['ttft_p50']:.3f}s, "
...
def run_budget_sweep(args, trace):
    ...
    for nb in blocks:
        if args.dry_run:
            ...
            agg = {"ttft_p50": 0.18 - 0.11 * hr, "throughput_tok_s": 1600 + 900 * hr,
                   "prefix_cache_hit_rate": float(hr), "n_ok": len(trace)}
        ...
        try:
            ...
            agg.update({k: v for k, v in scrape_metrics(base_url).items() if v is not None})
            points.append({"blocks": nb, **agg})
            print(f"  blocks={nb}: ttft_p50={agg['ttft_p50']:.3f}s "
...
def _write_budget(model, points):
    ...
    rows = [f"{p['blocks']} & {p['ttft_p50']:.3f} & {p['throughput_tok_s']:.1f} & "
            f"{(p.get('prefix_cache_hit_rate') or 0):.3f} \\\\" for p in points]
    tex = ("% vLLM KV-budget sweep (auto-generated by vllm_prefix_cache_eval.py)\n"
           "\\begin{tabular}{rrrr}\n\\toprule\n"
