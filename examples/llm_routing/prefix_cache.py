...
def tokenize_stream(prompts, encode, max_tokens: int):
    """Tokenize each prompt to a token-id tuple, truncated to ``max_tokens`` (cacheable head)."""
    stream = []
    for p in prompts:
...
def _plot(stationary, budgets, curves, shift_budget, meta, tag=""):
    ...
    axB.set_ylabel(
        "blocks reused per request (rolling)"
        if meta.get("real_order")
...
def main() -> None:
    ...
    ap.add_argument(
        "--tag", default="", help="suffix for output files (e.g. mooncake_conversation)"
    )
    ap.add_argument("--corpus-file", default="", help="local prompt log, one prompt per line")
    ap.add_argument(
        "--dataset", default="tatsu-lab/alpaca", help="HF dataset (if no --corpus-file)"
    )
    ...
    if args.mooncake_trace:
        try:
            ...
            meta = {
                "source": src,
                "tokenizer": "block-hash-ids",
                "n_prompts": len(stream),
                "real_order": True,
            }
        except Exception as e:  # noqa: BLE001
        ...
        try:
            ...
            meta = {
                "source": f"{args.chat_dataset} chat trace",
                "tokenizer": tok_name,
                "n_prompts": len(stream),
                "real_order": True,
            }
            ...
            print(
                f"Could not load chat dataset {args.chat_dataset} ({type(e).__name__}: {e}). "
