    ...
    with tarfile.open(tp, "r:gz") as t:
        for member in t:
            ...
            try:
                ...
                continue
            rec = {
                r["index"]: (float(r.get("score", 0.0)), float(r.get("cost", 0.0)))
...
def main() -> None:
    ...
    try:
        ...
        return
    print(
        f"LLMRouterBench (ArenaHard): {quality.shape[0]} prompts, {len(models)} models, "
    ...
    tex = (
        ...
        f"{quality.shape[0]} prompts, {len(models)} models, "
        f"{len(region_names)} task-variant regions.\n"
        "\\begin{tabular}{lcc}\n\\toprule\nPolicy & Quality & Cost/query (\\$) \\\\\n\\midrule\n"
        f"per-prompt oracle & {float(np.mean(oracle_q)):.3f} & --- \\\\\n"
        f"\\textbf{{region router ($\\lambda{{=}}0$)}} & "
        f"\\textbf{{{rq[0]:.3f}}} & {rc[0]:.5f} \\\\\n"
