...
def measure_real(items, region, cache, max_calls, max_spend, npz_cache, checkpoint_every=25):
    ...
    client = CachingLLMClient(
        BedrockClient(region=region, max_tokens=8),
        cache,
        max_calls=max_calls,
...
def _sweep_lambda(quality, in_tokens, n_subjects, lambdas, horizon=6000):
    ...
    for lam in lambdas:
        env = PrefixTreeRouting(
            2, depth, q, rel_costs, lam=lam, noise_std=0.05, rng=np.random.default_rng(0)
        )
        for strat, kw, key in [
...
def _write_outputs(items, quality, in_tokens, model, n_subjects, quiet=False):
    ...
    print(
        f"  adaptive(lam=0.3): acc={adapt_acc:.3f} tok={adapt_tok:.1f}  |  "
...
def _plot(acc, costs, frontier, model):
    ...
    for t in range(N_TRIM):
        ax.annotate(
            f"trim {t}", (costs[t], acc[t]), fontsize=7, xytext=(4, 4), textcoords="offset points"
        )
