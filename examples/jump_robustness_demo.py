...
def main() -> None:
    ...
    print(f"piecewise-smooth tree: {BRANCHING**DEPTH} leaves, {N_JUMPS} jumps, "
          f"horizon {HORIZON}, {N_SEEDS} seeds")
    for jw in WIDTHS:
        a, v, h = [], [], []
        for seed in range(N_SEEDS):
            env = TreeBandit.from_piecewise_smooth(
                BRANCHING, DEPTH, N_JUMPS, jump_width=jw, noise_std=0.1,
                rng=np.random.default_rng(seed)
            )
            a.append(run_hoo(env, HORIZON, SPREAD_SMOOTH, np.random.default_rng(100 + seed),
                             memory_bounded=True).final_regret)
            env2 = TreeBandit.from_piecewise_smooth(
                BRANCHING, DEPTH, N_JUMPS, jump_width=jw, noise_std=0.1,
                rng=np.random.default_rng(seed)
            )
            v.append(run_adaptive_variance(env2, HORIZON, np.random.default_rng(100 + seed)).final_regret)
            env3 = TreeBandit.from_piecewise_smooth(
                BRANCHING, DEPTH, N_JUMPS, jump_width=jw, noise_std=0.1,
                rng=np.random.default_rng(seed)
            )
            h.append(run_hybrid(env3, HORIZON, SPREAD_SMOOTH, np.random.default_rng(100 + seed),
                                jump_factor=0.5, n_min=8).final_regret)
        assumed.append(np.mean(a))
        datadriven.append(np.mean(v))
        hybrid.append(np.mean(h))
        print(f"  jump_width={jw:3d}: assumed-smooth={assumed[-1]:7.1f}  "
              f"data-driven={datadriven[-1]:7.1f}  hybrid={hybrid[-1]:7.1f}")

    fig, ax = plt.subplots(figsize=(7.8, 5))
    ax.plot(WIDTHS, assumed, "s-", color="#1f77b4", lw=2,
            label="assumed-smooth spread(level)")
    ax.plot(WIDTHS, datadriven, "o-", color="#d62728", lw=2,
            label="data-driven (within-cell variance)")
    ax.plot(WIDTHS, hybrid, "D-", color="#2ca02c", lw=2.4,
    ...
    ax.set_title(f"Robustness to {N_JUMPS} jump discontinuities\n"
