...
def _env(lm, seed):
    return TreeBandit(BRANCHING, DEPTH, leaf_means=lm, noise_std=0.1,
...
def run() -> dict:
    ...
    for s in range(N_SEEDS):
        ...
        for i, L in enumerate(L_SWEEP):
            r = run_hoo(_env(lm, 100 + s), HORIZON, lipschitz_spread(L, RHO),
                        np.random.default_rng(100 + s), memory_bounded=True)
...
def main() -> None:
    ...
    print(f"  assumed-L sweep final regret: " + "  ".join(
        f"L={L}:{a_mean[i]:.0f}" for i, L in enumerate(L_SWEEP)))
    print(f"  oracle-tuned L={oracle_L} -> {oracle_regret:.0f}")
    print(f"  estimated-L (ours)          -> {est_mean:.0f}  "
    ...
    axA.scatter([oracle_L], [oracle_regret], color=PALETTE["green"], s=90, zorder=5,
                marker="*", label="oracle-tuned L")
    axA.set_xscale("log")
    ...
    for key, col, lab in [("tight", PALETTE["blue"], f"assumed tight (L={L_SWEEP[0]})"),
