...
### C3 — Learned router: identifiability under episodic credit *(fixed on main's router)*
At review time the repo implemented only `TieredRouter`, a hand-coded
escalate-on-error heuristic, while the paper claimed a learned router. The main
branch independently added a cost-aware `ContextualUCBRouter`
(`qhat - lam*cost + UCB`), wired into `taubench_routing.py` — superseding the
review's stopgap implementation. What the review contributes on top: a genuine
identifiability failure found while testing the episodic credit assignment — with
deterministic tie-breaking, regions explore in lockstep, choices are perfectly
correlated across regions, and the shared terminal reward cannot identify per-region
means (they pin at the grand mean; the regional learner never separates from flat).
Fixed with a small epsilon-randomization (`explore_eps`, default 0.1) that
decorrelates the per-region designs; documented in the class docstring and covered by
`tests/test_routing.py` (including a test that reproduces the lockstep failure at
`explore_eps=0`). Set `explore_eps=0` for per-pull immediate-reward use where the
...
### C6 — ~8 scripts named in the paper did not exist *(resolved on main)*
At review time none of the named scripts existed in the reviewed branch. The main
branch now carries mature versions of all of them (`lipschitz_ablation.py`,
`theory_link.py`, `reasoning_search.py` with code benchmarks,
`reasoning_tree_lipschitz.py` with `trace_log` instrumentation, `prefix_cache.py`
with WildChat/LMSYS and Mooncake trace modes, `combine_reasoning_models.py`, and the
two vLLM systems scripts with `--dry-run` harness validation). The review's ports on
top: the offline SWE-bench Stage-1 gate (`examples/analysis/swebench_stage1.py`), the
scope phase diagram (`examples/reasoning/probe_scope_demo.py`), the MBPP probe-repair
check (`examples/reasoning/mbpp_probe_check.py`, built on `code_eval`), and the
Stage-1 gate helpers in `canopy.bandits.code_eval`
(`public_fraction`, `probe_truth_pairs`).
