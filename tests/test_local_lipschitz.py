...
def test_local_adaptive_beats_global_constants_on_heterogeneous_tree():
    ...
    for seed in range(6):
        ...
        local.append(
            run_local_lipschitz(e1, horizon, np.random.default_rng(100 + seed)).final_regret
        )
        e2 = TreeBandit(
            branching, depth, leaf_means=lm, noise_std=0.1, rng=np.random.default_rng(100 + seed)
        )
        g_tight.append(
            run_hoo(
                e2, horizon, tight, np.random.default_rng(100 + seed), memory_bounded=True
            ).final_regret
        )
        e3 = TreeBandit(
            branching, depth, leaf_means=lm, noise_std=0.1, rng=np.random.default_rng(100 + seed)
        ...
        g_loose.append(
            run_hoo(
                e3, horizon, loose, np.random.default_rng(100 + seed), memory_bounded=True
            ).final_regret
        )
