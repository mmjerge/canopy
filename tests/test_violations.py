...
from oco.bandits.rewards import hierarchical_gaussian_leaf_means


def test_violation_family_has_unique_optimum_and_decoys():
    lm = violation_family_leaf_means(4, 5, n_violations=8, violation_level=3,
...
def test_detect_violations_recovers_k_from_data():
    ...
    for k in (2, 8, 16):
        counts = []
        for seed in range(6):
            lm = violation_family_leaf_means(4, 5, n_violations=k, violation_level=3,
                                             rng=np.random.default_rng(seed))
            env = TreeBandit(4, 5, leaf_means=lm, noise_std=0.1,
                             rng=np.random.default_rng(900 + seed))
            counts.append(detect_violations(
                env, 3, floor, np.random.default_rng(900 + seed),
                n_samples_per_cell=250, jump_factor=1.5,
            ).count)
...
def test_detect_violations_quiet_on_smooth_tree():
    floor = lambda _l: 0.04  # noqa: E731
    lm = hierarchical_gaussian_leaf_means(4, 5, lambda lvl: 0.02 * 0.6**lvl,
...
def test_detect_and_relax_hybrid_beats_assume_smooth_on_spikes():
    ...
    from oco.bandits import HierarchicalTopK
    ...
    for s in range(12):
        ...
        def env(seed):
            return TreeBandit(b, d, leaf_means=lm, noise_std=0.05, probe_cost=0.05,
                              rng=np.random.default_rng(seed))

        e = env(40 + s)
        hi.append(HierarchicalTopK(400, 0.1, spread=0.3, beam_width=8).run(e, 1).evaluate(e, 1))
        det = detect_violations(env(80 + s), level, lambda _l: 0.08,
                                np.random.default_rng(80 + s), n_samples_per_cell=40).detected
        ranges = [(c * cell, (c + 1) * cell) for c in det]
        e = env(40 + s)
        hy.append(HierarchicalTopK(400, 0.1, spread=0.3, beam_width=8,
...
def test_edge_targeted_beats_blind_at_tight_budget():
    ...
    from oco.bandits import HierarchicalTopK, SuccessiveEliminationTopK
    from oco.bandits.rewards import adversarial_spike_leaf_means
    ...
    for s in range(16):
        ...
        det = detect_violations(env(80 + s), level, lambda _l: 0.08,
                                np.random.default_rng(80 + s), n_samples_per_cell=30).detected
        ranges = [(c * cell, (c + 1) * cell) for c in det]
        e = env(40 + s)
        edge.append(HierarchicalTopK(budget, 0.1, spread=0.3, beam_width=8,
                                     relaxed_ranges=ranges).run(e, 1).evaluate(e, 1))
